...
<div class="relative">
	<div class="vx-navbar-wrapper">
		<vs-navbar class="vx-navbar navbar-custom" :color="navbarColor" :class="classObj">
            ...
            <!-- CART DROPDOWN -->
            <vs-dropdown vs-custom-content vs-trigger-click class="cursor-pointer">
                <feather-icon icon="ShoppingCartIcon" class="cursor-pointer ml-4 mr-6 mt-1" :badge="serviceCartItems.length"></feather-icon>
                <vs-dropdown-menu class="cart-dropdown" :class="{'dropdown-custom': serviceCartItems.length}">

                    <!-- IF CART HAVE ITEMS: HEADER -->
                    <template v-if="serviceCartItems.length">
                        <div class="notification-header text-center p-5 bg-primary text-white">
                            <h3 class="text-white">{{ serviceCartItems.length }} Item<span v-show="cartItems.length > 1">s</span></h3>
                        ...
                        <ul class="bordered-items">
                            <li v-for="item in serviceCartItems" :key="item.s_id" class="vx-row no-gutter cart-item cursor-pointer">
                                ...
                                <div class="vx-col w-4/5 pr-4 pl-2 py-4 flex flex-col justify-center">
                                    <span class="font-medium block cart-item-title truncate">{{ item.s_title }}</span>
                                    <small class="truncate mb-2">{{ item.s_desc }}</small>
                                    <div class="flex items-center justify-between">
                                        <span class="text-sm font-medium">${{ item.s_cost }}</span>
			...
			<div class="the-navbar__user-meta flex items-center">
				<div class="text-right leading-tight hidden sm:block">
					<p class="font-semibold">{{ userDetails.a_name }}</p>
				</div>
				<vs-dropdown vs-custom-content vs-trigger-click class="cursor-pointer">
...
export default {
    ...
    computed: {
       ...mapState({ userDetails: state => state.auth.userDetails,
        serviceCartItems: state => state.service.serviceCartItems
       }),
        // HELPER
        sidebarWidth() {
