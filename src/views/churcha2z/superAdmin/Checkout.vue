    ...
    <div id="ecommerce-checkout-demo">
        <form-wizard
            ...
            <tab-content title="Cart" icon="feather icon-shopping-cart" class="mb-5">
                ...
                <!-- IF CART HAVE ITEMS -->
                <div class="vx-row" v-if="serviceCartItems.length">

                    <!-- LEFT COL -->
                    <div class="vx-col lg:w-2/3 w-full relative">
                        <div class="items-list-view" v-for="item in serviceCartItems" :key="item.s_id">
                            <item-list-view :item="item">

                                <!-- SLOT: ITEM META -->
                                <template slot="item-meta">
                                    <h6 class="item-name font-semibold mb-1">{{ item.s_title }}</h6>
                                    <p class="text-sm mb-2 cursor-pointer">{{ item.s_desc}} </p>
                                    ...
                                    <p class="text-success font-medium"> offers Available</p>
                                </template>
                    ...
                    <div class="vx-col lg:w-1/3 w-full">
                        <vx-card>
                            ...
                            <div v-for="item in serviceCartItems" :key="item.s_id" class="flex justify-between mb-2">
                                <span class="text-grey">{{item.s_title}}</span>
                                <span>${{item.s_cost}}</span>
                            </div>
                            <vs-divider />
                ...
                <vx-card title="You don't have any items in your cart." v-else>
                    <vs-button @click="$router.push('/index')">Browse Shop</vs-button>
                </vx-card>
...
const ItemListView = () => import('../apps/eCommerce/components/ItemListView.vue')
...
export default {
    ...
    computed: {
        ...
        ...mapGetters({
            totalPrize: getters => getters.service.totalCartPrize
        })
        ,
          isInCart() {
        ...
        serviceCartItems() {
            return this.$store.state.service.serviceCartItems.slice().reverse();
        },
        isInWishList() {
            return (itemId) => this.$store.getters['service/isInWishList'](itemId)
        },
    },
    methods: {
        ...
        removeItemFromCart(item) {
            this.$store.dispatch('service/toggleServiceInCart', item)
        },
        ...
        updateItemQuantity(event, index) {
            const itemIndex = Math.abs(index + 1 - this.serviceCartItems.length)
            this.$store.dispatch("service/updateItemQuantity", { quantity: event, index: itemIndex })
        },
